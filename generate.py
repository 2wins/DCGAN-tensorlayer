...
flags.DEFINE_integer("z_dim", 100, "Dimension of seed vector [64]")
flags.DEFINE_string("load_dir", "checkpoint/celebA_64", "Directory name to load the checkpoints [checkpoint/celebA]")
...
def main(_):
    ...
    if tl.files.folder_exists(FLAGS.load_dir) is False:
        raise ValueError("load_dir {} does not exist.".format(FLAGS.load_dir))
        
    with tf.device("/gpu:{}".format(FLAGS.gpu_num)):
        z = tf.placeholder(tf.float32, [1, FLAGS.z_dim], name='z_noise')

        net_g, _ = generator_simplified_api(z, is_train=False)
